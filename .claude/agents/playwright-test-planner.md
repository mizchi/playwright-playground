...
- The environment seed is `tests/seed.spec.ts`; it opens a deterministic local TODO app
  (Vite preview on http://localhost:4173) with an always-empty initial state.
- Always reference `**Seed:** tests/seed.spec.ts` in the generated plan.
- Assume a blank/fresh state at the start of every scenario.
